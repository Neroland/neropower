...
- **Player-data erasure** — a `PlayerDataEraser` registered with Core clears beam link-owner UUIDs
  ...
  stored field; NeroPower ships **no telemetry**.
...
- The unused bundled Sentry SDK from all three loader jars and the Sentry release step — NeroPower
  carries no crash reporting or analytics.
