#!/usr/bin/env python3
"""
Generate the NEROPOWER mod logo (square), in the shared Neroland family style
...
the focused energy mod, so the core-prism radiates YELLOW power spikes behind a bold lightning bolt Renders supersampled, then downsamples.

Outputs:
  art/logo/neropower_logo.png       (1024x1024 master)
  art/logo/neropower_logo_400.png   (CurseForge/Modrinth-ready)
  common/src/main/resources/neropower_logo.png  (256x256 in-game mods-list icon)
"""
SEED = 73
MOD = 'neropower'
NAME = 'NEROPOWER'
ACCENT = (255, 210, 62)
ACCENT_BRIGHT = (255, 236, 150)
BRIGHT = (255, 250, 224)
NEBULA = (120, 96, 18)
STAR_TINT = (255, 240, 176)
NAME_GLOW = (255, 210, 62, 255)
TAG_COL = (238, 218, 150, 255)
...
TAG = 'E N E R G Y   ·   G E N E R A T I O N   ·   G R I D'

def emblem_frame(base, cx, cy, rad):
    # radiant power spikes ring
    gl = Image.new("RGBA", (R, R), (0, 0, 0, 0)); gd = ImageDraw.Draw(gl)
    n = 16
    for i in range(n):
        a = math.radians(i * (360 / n))
        r0, r1 = rad * 1.22, rad * 1.7
        pa = a + math.pi / 2
        ww = rad * 0.055
        tip = (cx + math.cos(a) * r1, cy + math.sin(a) * r1)
        b1 = (cx + math.cos(a) * r0 + math.cos(pa) * ww, cy + math.sin(a) * r0 + math.sin(pa) * ww)
        b2 = (cx + math.cos(a) * r0 - math.cos(pa) * ww, cy + math.sin(a) * r0 - math.sin(pa) * ww)
        gd.polygon([tip, b1, b2], fill=ACCENT + (220,))
    base.alpha_composite(gl.filter(ImageFilter.GaussianBlur(SS)))
    base.alpha_composite(gl)
    # bold lightning bolt over the prism
    bl = Image.new("RGBA", (R, R), (0, 0, 0, 0)); bd = ImageDraw.Draw(bl)
    s = rad
    bolt = [(cx + 0.28 * s, cy - 1.15 * s), (cx - 0.42 * s, cy + 0.12 * s),
            (cx + 0.02 * s, cy + 0.12 * s), (cx - 0.24 * s, cy + 1.2 * s),
            (cx + 0.5 * s, cy - 0.18 * s), (cx + 0.06 * s, cy - 0.18 * s)]
    bd.polygon(bolt, fill=ACCENT_BRIGHT + (255,), outline=(60, 44, 8, 255), width=SS)
    base.alpha_composite(soft_glow(lambda dr: dr.polygon(bolt, fill=ACCENT + (200,)), 6 * SS))
